...
\begin{definition}{NFTA \cite{tata-nfta}}
	...
	If zero or more transition rules are applied, we write:
		$$t \moveStar t'$$
	If a given input tree \(t_{input}\) can be reduced to only a single state \(q \in Q_f\), then it is accepted by \(A\). The set of all input trees accepted by \(A\) is called the \textbf{Language of A}, which is denoted by \(L_A\).
\end{definition}
