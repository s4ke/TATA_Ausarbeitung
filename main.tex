...
\begin{minipage}{0.5\textwidth}
	...
	\end{tikzpicture}
	\\
	\centering \(false\) and \(true,\) \textit{according to our initial rules, are reduced to} \(false \rightarrow q_f\) \textit{and} \(true \rightarrow q_t\).
\end{minipage}
\\
\\
\\
\begin{minipage}{0.5\textwidth}
		...
		\end{tikzpicture}
	\\
	\centering \(or(q_f, q_t)\) \textit{and} \(or(q_t, q_t)\) are reduced to \(q_t\) each.
\end{minipage}
\begin{minipage}{0.5\textwidth}
	~~
	\begin{tikzpicture}[auto]
